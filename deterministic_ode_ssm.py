...
class DGSSM(pytorch_lightning.LightningModule):
    ...
    def forward(self, x):
        ...
        zt = odeint(self.ode_func, z0, t, method='rk4', options={'step_size': 1})  # [T,q]
        zt = zt.permute([1, 0, 2])
        ...
        if self.current_epoch % 10 == 0:
            ...
            for tup in outputs:
                if recons is None:
                    ...
                    recons = torch.vstack((recons, tup["val_tmps"]))
...
def parse_args():
    ...
    # Experiment ID
    parser.add_argument('--exptype', type=str, default='default', help='name of the exp folder')
    parser.add_argument('--checkpt', type=str, default='None', help='checkpoint to resume training from')
    parser.add_argument('--model', type=str, default='mix', help='which model to choose')

    parser.add_argument('--random', type=bool, default=False, help='whether to have randomized sequence starts')

    # Learning hyperparameters
    parser.add_argument('--num_epochs', type=int, default=250, help='number of epochs to run over')
    ...
    parser.add_argument('--r_beta', type=float, default=5, help='multiplier for x0 bce term in loss')
    return parser
...
if __name__ == '__main__':
    ...
    # Input generation
    traindata = DynamicsDataset(split='train', random=arg.random)
    ...
    valdata = DynamicsDataset(split='val', random=arg.random)
